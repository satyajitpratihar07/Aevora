...
export const NurseLoginPage: React.FC<NurseLoginPageProps> = ({ onBackToRoles, onSuccessLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (password && password.length >= 6) {
        try {
          await loginWithFirebaseEmail(nurseId, password, 'NURSE');
        } catch {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError('');
    try {
      const success = await loginWithGoogle('NURSE');
      if (success) {
        onSuccessLogin();
      } else {
  ...
  const handleSelectGoogleAccount = async (email: string, name: string) => {
    await loginWithGoogle('NURSE', email, name);
    onSuccessLogin();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <footer className="relative z-10 py-4 text-center text-xs text-slate-500 border-t border-slate-200 bg-white">
        AVORA HMS Staff Nurse Portal · Dedicated Inpatient Telemetry Node
      </footer>
